...
from akpi.auth_tools.sussion_client.sussion import *
    ...
    def init_clients(cls, Authzk_Hosts):
        ...
        with cls._instance_lock:
            ...
            if (cls.zk.client_state != KeeperState.CONNECTED and cls.zk.client_state != KeeperState.CONNECTED_RO):
                cls.zk.restart()
            children = cls.zk.get_children("/com/akulaku/sussion", watch=cls.children_callback)
            for child in children:
                info = cls.zk.get("/com/akulaku/sussion/" + child)
                ...
                try:
                    ...
                    root_logger.info("thrift client open!")
                except Thrift.TException as ex:
                    root_logger.error("thrift connect error:%s" % (ex.message))

    @classmethod
    def children_callback(cls, event):
        root_logger.info("SussionClient children_callback event {}".format(event))
